...
namespace KGERP.Service.Implementation.Procurement
{
    public class ProcurementService
    {
        ...
        public async Task<VMSalesOrderSlave> GetCustomerSalesOrderDetails(int companyId, int orderMasterId)
        {
            VMSalesOrderSlave vmSalesOrderSlave = new VMSalesOrderSlave();
            vmSalesOrderSlave = await Task.Run(() => (from t1 in _db.OrderMasters.Where(x => x.IsActive && x.OrderMasterId == orderMasterId && x.CompanyId == companyId)
        ...
        public async Task<VMSalesOrderSlave> GetSingleOrderDetails(int id)
        {
            var v = await Task.Run(() => (from t1 in _db.OrderDetails
                                          ...
                                          join t3 in _db.ProductCategories on t4.ProductCategoryId equals t3.ProductCategoryId
        ...
        public async Task<long> FoodCustomerOrderMasterAdd(VMSalesOrderSlave vmSalesOrderSlave)
        {
            long result = -1;
            ...
            OrderMaster orderMaster = new OrderMaster
            {
                ...
                OrderNo = soCid,
                OrderDate = vmSalesOrderSlave.OrderDate,
                CustomerId = vmSalesOrderSlave.CustomerId, //Customer
                ExpectedDeliveryDate = vmSalesOrderSlave.ExpectedDeliveryDate,
                PaymentMethod = vmSalesOrderSlave.CustomerPaymentMethodEnumFK,
                ProductType = "F",
                Status = (int)EnumSOStatus.Draft,
        ...
        public async Task<long> FoodCustomerOrderMasterEdit(VMSalesOrder vmSalesOrder)
        {
            long result = -1;
            OrderMaster orderMaster = await _db.OrderMasters.FindAsync(vmSalesOrder.OrderMasterId);
        ...
        public async Task<long> FoodCustomerOrderDetailAdd(VMSalesOrderSlave vmSalesOrderSlave)
        {
            long dateTime = DateTime.Now.Ticks;
            long result = -1;
        ...
        public async Task<int> FoodCustomerOrderDetailEdit(VMSalesOrderSlave vmSalesOrderSlave)
        {
            var result = -1;
        ...
        public async Task<VMSalesOrderSlave> GetFoodCustomerSalesOrderDetails(int companyId, int orderMasterId)
        {
            VMSalesOrderSlave vmSalesOrderSlave = new VMSalesOrderSlave();
            vmSalesOrderSlave = await Task.Run(() => (from t1 in _db.OrderMasters.Where(x => x.IsActive && x.OrderMasterId == orderMasterId && x.CompanyId == companyId)
                                                      join t2 in _db.Vendors on t1.CustomerId equals t2.VendorId
        ...
        public async Task<VMSalesOrder> GetFoodCustomerOrderMasterById(int orderMasterId)
        {

            var v = await Task.Run(() => (from t1 in _db.OrderMasters.Where(x => x.IsActive && x.OrderMasterId == orderMasterId && x.CustomerId > 0)
                                          join t2 in _db.Vendors on t1.CustomerId equals t2.VendorId
                                          join t3 in _db.Companies on t1.CompanyId equals t3.CompanyId
        ...
        public async Task<VMSalesOrder> GetFoodCustomerOrderMasterList(int companyId, DateTime? fromDate, DateTime? toDate, int? vStatus)
        {
            VMSalesOrder vmSalesOrder = new VMSalesOrder();
            vmSalesOrder.CompanyFK = companyId;
        ...
        public async Task<int> VendorDepositSubmit(int vendorDepositId)
        {
            ...
            //var costCenter = _db.Accounting_CostCenter.FirstOrDefault(x => x.CompanyId == vendor.CompanyId);
            //DateTime voucherDate = vendorDeposit.DepositDate;
            //int voucherTypeId = (int)SeedJournalEnum.CreditVoucher;
            //VoucherType voucherType = _db.VoucherTypes.FirstOrDefault(x => x.VoucherTypeId == voucherTypeId);
            //string voucherNo = string.Empty;
            //int vouchersCount = _db.Vouchers.Count(x => x.VoucherTypeId == voucherTypeId && x.CompanyId == vendor.CompanyId
            //    && x.VoucherDate.Value.Month == voucherDate.Month
